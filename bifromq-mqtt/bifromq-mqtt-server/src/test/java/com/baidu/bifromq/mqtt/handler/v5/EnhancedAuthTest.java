...
public class EnhancedAuthTest extends MockableTest {
    ...
    public void testAuthAgainBeforeServerReply() {
        ...
        when(authProvider.extendedAuth(any(MQTT5ExtendedAuthData.class))).then(invocation -> {
            Thread.sleep(100);
            CompletableFuture<MQTT5ExtendedAuthResult> future = new CompletableFuture<>();
            future.completeAsync(() -> MQTT5ExtendedAuthResult.newBuilder()
                    .setContinue(Continue.newBuilder()
                            .setAuthData(ByteString.copyFromUtf8(challenge))
                            .build())
                    .build());
