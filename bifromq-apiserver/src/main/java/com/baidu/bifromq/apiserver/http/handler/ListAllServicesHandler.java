...
@Slf4j
@Path("/landscape/service/all")
public class ListAllServicesHandler implements IHTTPRequestHandler {
    private final IRPCServiceTrafficService trafficService;

    public ListAllServicesHandler(IRPCServiceTrafficService trafficService) {
    ...
    public CompletableFuture<FullHttpResponse> handle(long reqId, FullHttpRequest req) {
        ...
        return trafficService.services()
            .firstElement()
            .toCompletionStage()
            .toCompletableFuture()
            .thenApply(serviceUniqueNames -> {
