...
final class InProcTransport implements ServerTransport, ConnectionClientTransport {
    ...
    public synchronized Runnable start(Listener listener) {
        ...
        if (optionalServerListener.isPresent()) {
            ...
            InProcServer server = InProcServer.findServer(address);
            if (server != null) {
    ...
    private class InProcessStream {
        ...
        private class InProcessClientStream implements ClientStream {
            ...
            public void start(ClientStreamListener listener) {
                ...
                synchronized (InProcTransport.this) {
                    statsTraceCtx.clientOutboundHeaders();
                    streams.add(InProcTransport.InProcessStream.this);
                    if (GrpcUtil.shouldBeCountedForInUse(callOptions)) {
                        inUseState.updateObjectInUse(InProcTransport.InProcessStream.this, true);
                    }
    ...
    /**
     * Returns a new status with the same code and description. If includeCauseWithStatus is true, cause is also
     * included.
     *
     * <p>For InProcess transport to behave in the same way as the other transports,
     * when exchanging statuses between client and server and vice versa, the cause should be excluded from the status.
