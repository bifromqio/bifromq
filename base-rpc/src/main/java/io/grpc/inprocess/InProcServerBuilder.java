...
public final class InProcServerBuilder extends ForwardingServerBuilder<InProcServerBuilder> {
     ...
     * @return a new builder
     */
    public static InProcServerBuilder forAddress(SocketAddress listenAddress) {
        assert listenAddress instanceof InProcSocketAddress;
        return new InProcServerBuilder((InProcSocketAddress) listenAddress);
    }
    ...
    public static InProcServerBuilder forPort(int port) {
        throw new UnsupportedOperationException("call forName() instead");
    }

    private final ServerImplBuilder serverImplBuilder;
    final InProcSocketAddress listenAddress;
    ...
    private InProcServerBuilder(InProcSocketAddress listenAddress) {
     ...
     * @since 1.11.0
     */
    public InProcServerBuilder scheduledExecutorService(
    ...
    /**
     * Provides a custom deadline ticker that this server will use to create incoming {@link Deadline}s.
     *
     * <p>This is intended for unit tests that fake out the clock.  You should also have a fake {@link
     * ScheduledExecutorService} whose clock is synchronized with this ticker and set it to
     * {@link #scheduledExecutorService}. DO NOT use this in production.
     *
     * @return this
     * @see Deadline#after(long, TimeUnit, Deadline.Ticker)
     * @since 1.24.0
     */
    public InProcServerBuilder deadlineTicker(Deadline.Ticker ticker) {
        serverImplBuilder.setDeadlineTicker(ticker);
        return this;
    }

    /**
     * Sets the maximum size of metadata allowed to be received. {@code Integer.MAX_VALUE} disables the enforcement.
     * Defaults to no limit ({@code Integer.MAX_VALUE}).
     *
     * <p>There is potential for performance penalty when this setting is enabled, as the Metadata
     * must actually be serialized. Since the current implementation of Metadata pre-serializes, it's currently
     * negligible. But Metadata is free to change its implementation.
     *
     ...
     */
    @Override
    public InProcServerBuilder maxInboundMetadataSize(int bytes) {
        Preconditions.checkArgument(bytes > 0, "maxInboundMetadataSize must be > 0");
        this.maxInboundMetadataSize = bytes;
        return this;
    }

    InProcServer buildTransportServers(List<? extends ServerStreamTracer.Factory> streamTracerFactories) {
        return new InProcServer(this, streamTracerFactories);
    }

    @Override
    public InProcServerBuilder useTransportSecurity(File certChain, File privateKey) {
