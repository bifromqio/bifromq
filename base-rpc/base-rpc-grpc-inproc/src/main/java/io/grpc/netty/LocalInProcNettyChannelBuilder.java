...
public final class LocalInProcNettyChannelBuilder extends ForwardingChannelBuilder2<LocalInProcNettyChannelBuilder> {
    ...
    /**
     * If true, indicates that the transport may use the GET method for RPCs, and may include the request body in the
     * query params.
     */
     ...
     * Channel types other than SocketChannel. {@link #forAddress(String, int)} should generally be preferred over this
     * method, since that API permits delaying DNS lookups and noticing changes to DNS. If an unresolved
     * InetSocketAddress is passed in, then it will remain unresolved.
     */
    public static LocalInProcNettyChannelBuilder forAddress(SocketAddress serverAddress) {
        return new LocalInProcNettyChannelBuilder(serverAddress);
    }

    /**
     * Creates a new builder with the given server address. This factory method is primarily intended for using Netty
     * Channel types other than SocketChannel. {@link #forAddress(String, int, ChannelCredentials)} should generally be
     * preferred over this method, since that API permits delaying DNS lookups and noticing changes to DNS. If an
    ...
    public static LocalInProcNettyChannelBuilder forAddress(SocketAddress serverAddress,
        ...
        return new LocalInProcNettyChannelBuilder(serverAddress, creds, result.callCredentials, result.negotiator);
    }

    /**
     * Creates a new builder with the given host and port.
     */
    public static LocalInProcNettyChannelBuilder forAddress(String host, int port) {
     ...
     * Creates a new builder with the given host and port.
     */
    public static LocalInProcNettyChannelBuilder forAddress(String host, int port, ChannelCredentials creds) {
        return forTarget(GrpcUtil.authorityFromHostAndPort(host, port), creds);
    }

    /**
     * Creates a new builder with the given target string that will be resolved by {@link io.grpc.NameResolver}.
     */
    public static LocalInProcNettyChannelBuilder forTarget(String target) {
        return new LocalInProcNettyChannelBuilder(target);
    }

    /**
     * Creates a new builder with the given target string that will be resolved by {@link io.grpc.NameResolver}.
     */
    public static LocalInProcNettyChannelBuilder forTarget(String target, ChannelCredentials creds) {
        FromChannelCredentialsResult result = ProtocolNegotiators.from(creds);
        if (result.error != null) {
            throw new IllegalArgumentException(result.error);
        }
        return new LocalInProcNettyChannelBuilder(target, creds, result.callCredentials, result.negotiator);
    }
    ...
    LocalInProcNettyChannelBuilder(String target) {
    ...
    LocalInProcNettyChannelBuilder(
    ...
    LocalInProcNettyChannelBuilder(SocketAddress address) {
     ...
     * {@link NioSocketChannel} must use {@link io.netty.channel.nio.NioEventLoopGroup}, otherwise your application
     * won't start.
     */
    ...
    /**
     * Specifies the {@link ChannelFactory} to create {@link Channel} instances. This method is usually only used if the
     * specific {@code Channel} requires complex logic which requires additional information to create the
     * {@code Channel}. Otherwise, recommend to use {@link #channelType(Class)}.
     *
     * <p>It's an optional parameter. If the user has not provided an Channel type or ChannelFactory
     * when the channel is built, the builder will use the default one which is static.
     *
     * <p>You must also provide corresponding {@link #eventLoopGroup(EventLoopGroup)}. For example,
     * {@link NioSocketChannel} based {@link ChannelFactory} must use {@link io.netty.channel.nio.NioEventLoopGroup},
    ...
    /**
     * Specifies a channel option. As the underlying channel as well as network implementation may ignore this value
     * applications should consider it a hint.
     */
    @CanIgnoreReturnValue
    public <T> LocalInProcNettyChannelBuilder withOption(ChannelOption<T> option, T value) {
     ...
     */
    @CanIgnoreReturnValue
    public LocalInProcNettyChannelBuilder negotiationType(NegotiationType type) {
     ...
     * #channelFactory(ChannelFactory)} corresponding to the given {@code EventLoopGroup}. For example,
     * {@link io.netty.channel.nio.NioEventLoopGroup} requires {@link NioSocketChannel}
     *
     * <p>The channel won't take ownership of the given EventLoopGroup. It's caller's responsibility
     * to shut it down when it's desired.
     */
    @CanIgnoreReturnValue
    public LocalInProcNettyChannelBuilder eventLoopGroup(@Nullable EventLoopGroup eventLoopGroup) {
    ...
    @CanIgnoreReturnValue
    LocalInProcNettyChannelBuilder eventLoopGroupPool(ObjectPool<? extends EventLoopGroup> eventLoopGroupPool) {
        this.eventLoopGroupPool = checkNotNull(eventLoopGroupPool, "eventLoopGroupPool");
        return this;
    }

    /**
     * SSL/TLS context to use instead of the system default. It must have been configured with {@link GrpcSslContexts},
     * but options could have been overridden.
     */
    @CanIgnoreReturnValue
    public LocalInProcNettyChannelBuilder sslContext(SslContext sslContext) {
        checkState(!freezeProtocolNegotiatorFactory,
            "Cannot change security when using ChannelCredentials");
        if (sslContext != null) {
    ...
    /**
     * Sets the initial flow control window in bytes. Setting initial flow control window enables auto flow control
     * tuning using bandwidth-delay product algorithm. To disable auto flow control tuning, use
     * {@link #flowControlWindow(int)}. By default, auto flow control is enabled with initial flow control window size
     * of {@link #DEFAULT_FLOW_CONTROL_WINDOW}.
     */
    @CanIgnoreReturnValue
    public LocalInProcNettyChannelBuilder initialFlowControlWindow(int initialFlowControlWindow) {
    ...
    /**
     * Sets the flow control window in bytes. Setting flowControlWindow disables auto flow control tuning; use
     * {@link #initialFlowControlWindow(int)} to enable auto flow control tuning. If not called, the default value is
     * {@link #DEFAULT_FLOW_CONTROL_WINDOW}) with auto flow control tuning.
     */
    @CanIgnoreReturnValue
    public LocalInProcNettyChannelBuilder flowControlWindow(int flowControlWindow) {
    ...
    /**
     * Sets the maximum size of header list allowed to be received. This is cumulative size of the headers with some
     * overhead, as defined for
    ...
    public LocalInProcNettyChannelBuilder maxHeaderListSize(int maxHeaderListSize) {
        return maxInboundMetadataSize(maxHeaderListSize);
    }

    /**
     * Sets the maximum size of metadata allowed to be received. This is cumulative size of the entries with some
     * overhead, as defined for
     * <a href="http://httpwg.org/specs/rfc7540.html#rfc.section.6.5.2">
     * HTTP/2's SETTINGS_MAX_HEADER_LIST_SIZE</a>. The default is 8 KiB.
     *
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder maxInboundMetadataSize(int bytes) {
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder usePlaintext() {
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder useTransportSecurity() {
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder keepAliveTime(long keepAliveTime, TimeUnit timeUnit) {
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder keepAliveTimeout(long keepAliveTimeout, TimeUnit timeUnit) {
    ...
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder keepAliveWithoutCalls(boolean enable) {
    ...
    public static class LocalSocketPicker {
         ...
         * @param attrs         the Attributes present on the {@link EquivalentAddressGroup} associated with the
         *                      address.
    ...
    /**
     * Sets the maximum message size allowed for a single gRPC frame. If an inbound messages larger than this limit is
     * received it will not be processed and the RPC will fail with RESOURCE_EXHAUSTED.
     */
    @CanIgnoreReturnValue
    @Override
    public LocalInProcNettyChannelBuilder maxInboundMessageSize(int max) {
    ...
    ClientTransportFactory buildTransportFactory() {
        ...
        return new LocalInProcNettyTransportFactory(
    ...
    @CanIgnoreReturnValue
    LocalInProcNettyChannelBuilder disableCheckAuthority() {
        this.managedChannelImplBuilder.disableCheckAuthority();
        return this;
    }

    @CanIgnoreReturnValue
    LocalInProcNettyChannelBuilder enableCheckAuthority() {
    ...
    @CanIgnoreReturnValue
    @VisibleForTesting
    LocalInProcNettyChannelBuilder setTransportTracerFactory(TransportTracer.Factory transportTracerFactory) {
     ...
     */
    private static final class LocalInProcNettyTransportFactory implements ClientTransportFactory {
        ...
        public ConnectionClientTransport newClientTransport(
            ...
            // create in-proc transport for local in-proc RPC
            if (serverAddress instanceof InProcSocketAddress) {
                return new InProcTransport(
                    serverAddress, maxHeaderListSize, options.getAuthority(), options.getUserAgent(),
                    options.getEagAttributes(), false);
            }
        ...
        public SwapChannelCredentialsResult swapChannelCredentials(ChannelCredentials channelCreds) {
            ...
            ClientTransportFactory factory = new LocalInProcNettyTransportFactory(
